    ...
    public partial class Track : Form
    {
        ...
        private void TmonitorElapsed(object sender, ElapsedEventArgs e)
        {
            try
            {
                ...
                writelog("p.ProcessName: " + p.ProcessName, string.Empty);

                appltitle = APIFuncs.ActiveApplTitle().Trim().Replace("\0", "");

                writelog("appltitle: " + appltitle, string.Empty);
        ...
        private void addelement(string app, string title, double time, double notime)
        {
            try
            {
                writelog("addelement: --- " + app + " -- " + title, "addelement");
                ...
                writelog("cantidad elementos: --- " + lmt.Count(), "addelement");
            }
            catch (Exception ex)
            {
        ...
        private async Task<Response> CallApiCapture(string rute, object obj, string token)
        {
            try
            {
                ...
                //local
                clientcapture.BaseAddress = new Uri("http://194.113.72.223:8090/Api/");
            ...
            {
                writelog(ex.Message + " --- " + ex.InnerException.Message, "Error CallApiCapture()");
            }
            return null;
        }
